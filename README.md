...
send_large_msg(receiver, " ["..user_id.."]  تــم الـغاء الـكـتـم ")
    ...
    local data = load_data(_config.moderation.data)
        ...
        if matches[1] == 'ضع تكرار' then
            ...
            if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 20 then
                return "ضــع الـتـكـرار مـن 5الـئ 20"
            end
            local flood_max = matches[2]
            data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
            save_data(_config.moderation.data, data)
            savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
            return 'تــــم تـعـين عـدد الـتـكـرار : '..matches[2]
        end
  ...
  if matches[1] == 'قفل' and is_owner(msg) then
   ...
   local msg_type = 'Audio'
    if not is_muted(chat_id, msg_type..': yes') then
     ...
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــــصــــوت    \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Photo'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــــصــــور   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Video'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــــفــيــديــــو   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Gifs'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــــصــــور  الــــــمتــــحــــركــــﮥ   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Documents'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــفــايــلات   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Text'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــدردشــــة  \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'All'
    if not is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
     mute(chat_id, msg_type)
                    return 'تــــم✔️قــــفــــل الــــــمــــجــــمــــوعــة    \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
  ...
  if matches[1] == 'فتح' and is_momod(msg) then
   ...
   local msg_type = 'Audio'
    if is_muted(chat_id, msg_type..': yes') then
     ...
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــــصــــوت    \nبواسطه     (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Photo'
    if is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــــصــــور   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Video'
    if is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــــفــيــديــــو   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Gifs'
    if is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــــصــــور  الــــــمتــــحــــركــــﮥ  \nبواسطه     (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Documents'
    if is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــفــايــلات   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'Text'
    if is_muted(chat_id, msg_type..': yes') then
     ...
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــدردشــــة   \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
   ...
   local msg_type = 'All'
    if is_muted(chat_id, msg_type..': yes') then
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
     unmute(chat_id, msg_type)
                    return 'تــــم✔️فــــتــــح الــــــمــــجــــمــــوعــة    \nبواسطه      (@'..(msg.from.username or 'لا يوجد')..')\n'
                else
        ...
        if matches[1] == "كتم" and is_momod(msg) then
            ...
            elseif matches[1] == "كتم" and string.match(matches[2], '^%d+$') then
                ...
                if is_muted_user(chat_id, user_id) then
                    ...
                    return "["..user_id.."] تـم الـغـاء الـكـتـم"
                elseif is_momod(msg) then
                    mute_user(chat_id, user_id)
                    savelog(msg.to.id, name_log.." ["..msg.from.id.."] added ["..user_id.."] to the muted users list")
                    return "["..user_id.."] تـم الـكـتـم"
                end
